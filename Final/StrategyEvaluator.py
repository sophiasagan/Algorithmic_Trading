...
class StrategyEvaluator:
    ...
    def __init__(self, strategy_function, strategy_settings:dict={'indicator':['low_boll', 'fast_sma', 'slow_sma']}):
    ...
    for i in range(0, len(df['close '])-1):
        ...
        if strategy_result:
            ...
            if df['low'][i] < stop_loss_price:
                ...
                sell_times.append([df['date'][i], stop_loss_price])
        ...
        self.results[model.symbol] = dict(
            returns = round(Decimal(100.00) * (resulting_balance/Decimal(starting_balance) - Decimal(1.0)), 3),
            buy_times = buy_times,
    ...
    def printResults(self):
        ...
        print("Unprofitable Symbols: "+str(self.profitable_symbols))

        if len(self.profits_list) > 0:
            profitability = Decimal(100.00) * (self.complete_resulting_balance/self.complete_starting_balance - Decimal(1.0))
